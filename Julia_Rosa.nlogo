...
to transform-world ;calcular pendientes y hacer barrido
  set mx (re-max - re-min) / (max-pxcor - min-pxcor) ;Pendiente en el eje de los reales
  set my (im-max - im-min) / (max-pycor - min-pycor) ;Pendiente en el eje de los imaginarios
...
to iteration ;( ultimo)agarrar parte imaginaria de cada parche
  ...
  [
    set aux a
    set a a ^ 2 - b ^ 2 + r1 ;parte real
    set b 2 * aux * b + i1 ;parte imaginaria
